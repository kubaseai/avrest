 ...
 */
public class Http11IcapInputBuffer implements InputBuffer, ApplicationBufferHandler {

    // -------------------------------------------------------------- Constants

    private static final Logger log = LoggerFactory.getLogger(Http11IcapInputBuffer.class);

    private final static String PROT_ICAP = "ICAP/1.0";
    ...
    public Http11IcapInputBuffer(Request request, int headerBufferSize, boolean rejectIllegalHeader,
    ...
    private boolean HttpParser_isHttpProtocol(byte chr) {
        ...
        if (parsingRequestLinePhase == 6) {
            ...
            while (!parsingRequestLineEol) {
                ...
                if (chr == Constants.CR) {
                    ...
                    parsingRequestLineEol = true;
                } else if (prevChr == Constants.CR || !HttpParser_isHttpProtocol(chr)) {
            ...
            if (end - parsingRequestLineStart > 0) {
                ...
                isIcap = request.protocol().equals("ICAP/1.0");
                parsingRequestLinePhase = 7;
            }
    ...
    private String readLine(ByteBuffer byteBuffer, boolean block) throws IOException {
        ...
        int nextLine = byteBuffer.position();
        int lastTextPos = -1;
        if (prevChr == Constants.CR) {
            // Must have read two bytes (first was CR, second was not LF)
            lastTextPos = nextLine - 2;
        }
        else {
            // Must have only read one byte
            lastTextPos = nextLine - 1;
        }
        String line = new String(byteBuffer.array(), start, lastTextPos - start);
        byteBuffer.position(nextLine);
        return line;
    }
    ...
    boolean parseHeaders() throws IOException {
        ...
        if (icapAuthz!=null && auth==null) {
            headers.addValue("Authorization").setString(icapAuthz);
        }
        return status;
    }

    private void prepareForRepeatedHeaderParsing() {
        parsingHeader = true;
        headerParsePos = HeaderParsePosition.HEADER_START;
        headerData.recycle();
    }
     ...
     */
    boolean _parseHeaders() throws IOException {
    ...
    private String parseInvalid(int startPos, ByteBuffer buffer) {
        ...
        String result = toPrintableString(buffer.array(), buffer.arrayOffset() + startPos,
    ...
    private HeaderParseStatus parseHeader() throws IOException {
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_NAME) {
            ...
            if (chr == Constants.COLON) {
                ...
                if (log.isDebugEnabled()) {
                    String hdrName = new String(byteBuffer.array(), headerData.start, pos - headerData.start);
                    log.debug("Hdr "+hdrName);
                }
    ...
    ///
    private static String toPrintableString(byte[] bytes, int offset, int len) {
