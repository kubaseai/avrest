...
public class Http11IcapProcessor extends AbstractProcessor {

    private static final Logger log = LoggerFactory.getLogger(Http11IcapProcessor.class);
     ...
     * Input.
     */
    private final Http11IcapInputBuffer inputBuffer;


    /**
     * Output.
     */
    private final Http11IcapOutputBuffer outputBuffer;
    ...
    public Http11IcapProcessor(AbstractHttp11Protocol<?> protocol, Adapter adapter) {
        super(adapter);
        this.protocol = protocol;

        httpParser = new HttpParser(protocol.getRelaxedPathChars(), protocol.getRelaxedQueryChars());

        inputBuffer = new Http11IcapInputBuffer(request, protocol.getMaxHttpRequestHeaderSize(),
                protocol.getRejectIllegalHeader(), httpParser);
        request.setInputBuffer(inputBuffer);

        outputBuffer = new Http11IcapOutputBuffer(response, protocol.getMaxHttpResponseHeaderSize());
        ...
        inputBuffer.addFilter(
                new ChunkedInputFilter(protocol.getMaxTrailerSize(), Set.of("Expires"),
    ...
    private void prepareRequestProtocol() {
        ...
        } else if (protocolMB.equals("ICAP/1.0")) {
            http09 = false;
            http11 = true;
            keepAlive = true;            
    ...
    private void prepareRequest() throws IOException {
        ...
        if (uriBC.startsWithIgnoreCase("http", 0) || uriBC.startsWithIgnoreCase("icap", 0)) {
            int pos = 4;
            // Check for https
    ...
    private final static Log logWrapper = getLog(log);
    ...
    private final static Log getLog(Logger log) {
        return new Log() {
    ...
    protected final void setRequestBody(ByteChunk body) {
        ...
        Http11IcapInputBuffer internalBuffer = (Http11IcapInputBuffer) request.getInputBuffer();
